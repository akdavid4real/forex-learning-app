...
export function LiveLearningProgress() {
  ...
  const completed = progress.filter((item) => Boolean(item.completed_at)).length;
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.hero}>
        ...
        <Text style={styles.heroLabel}>course completion</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  title: { color: colors.mainText, fontWeight: '900', fontSize: 30, lineHeight: 36 },
  muted: { color: colors.mutedText, lineHeight: 21 },
  hero: { backgroundColor: colors.surface, borderRadius: 22, padding: 22, alignItems: 'center' },
  percent: { color: colors.primary, fontWeight: '900', fontSize: 46 },
  heroLabel: { color: colors.mutedText, marginTop: 4 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  metric: { width: '48%', flexGrow: 1, backgroundColor: colors.surface, borderRadius: 16, padding: 16, minHeight: 94, justifyContent: 'center' },
  metricValue: { color: colors.mainText, fontWeight: '900', fontSize: 22 },
  metricLabel: { color: colors.mutedText, marginTop: 4, fontSize: 12 },
});

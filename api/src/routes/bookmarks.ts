...
export function createBookmarkRoutes(services: AppServices): FastifyPluginAsync {
  return async function bookmarkRoutes(app) {
    app.get('/', { preHandler: requireActiveLearner(services) }, async (request) => {
    ...
    app.put('/:lessonId', { preHandler: requireActiveLearner(services) }, async (request, reply) => {
    ...
    app.delete('/:lessonId', { preHandler: requireActiveLearner(services) }, async (request, reply) => {
      const { lessonId } = request.params as { lessonId: string };

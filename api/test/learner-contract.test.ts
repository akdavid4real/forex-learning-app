...
const activeTables: TableData = {
  profiles: [{ id: 'user-1', display_name: 'Learner', avatar_url: null, xp: 10, current_streak: 1, longest_streak: 2, access_status: 'active' }],
...
describe('learner data contract', () => {
  ...
  beforeAll(async () => {
    app = await createApp({ environment, services: servicesWithTables(activeTables) });
  });
  ...
  test('learner can update their display name', async () => {
    const response = await app.inject({
      headers: { authorization: 'Bearer test-token' },
      method: 'PATCH',
      payload: { display_name: 'David Learner' },
      url: '/api/v1/users/me',
    });
    expect(response.statusCode).toBe(200);
    expect(response.json().profile.id).toBe('user-1');
  });

  test('invalid display name is rejected', async () => {
    const response = await app.inject({
      headers: { authorization: 'Bearer test-token' },
      method: 'PATCH',
...
describe('learner access gate', () => {
  let app: FastifyInstance;

  beforeAll(async () => {
    app = await createApp({
      environment,
      services: servicesWithTables({ ...activeTables, profiles: [{ id: 'user-1', access_status: 'pending' }] }),
    });
  });

  afterAll(async () => { await app.close(); });

  test('pending learners cannot read persisted learning progress', async () => {
    const response = await app.inject({ headers: { authorization: 'Bearer test-token' }, method: 'GET', url: '/api/v1/progress' });
    expect(response.statusCode).toBe(403);
    expect(response.json()).toMatchObject({ access_status: 'pending' });
  });
});
